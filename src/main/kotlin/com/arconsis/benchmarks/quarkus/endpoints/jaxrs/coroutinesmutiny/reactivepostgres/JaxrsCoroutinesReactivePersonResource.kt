package com.arconsis.benchmarks.quarkus.endpoints.jaxrs.coroutinesmutiny.reactivepostgres

import com.arconsis.benchmarks.quarkus.dto.PersonDto
import com.arconsis.benchmarks.quarkus.endpoints.jaxrs.coroutinesmutiny.CoroutineScopedResource
import com.arconsis.benchmarks.quarkus.service.reactive.coroutines.CoroutinesPersonService
import io.smallrye.mutiny.Uni
import java.util.*
import javax.inject.Inject
import javax.ws.rs.*
import javax.ws.rs.core.MediaType

@Path("/frameworks/jaxrs/type/coroutines/database/reactive")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
class JaxrsCoroutinesReactivePersonResource : CoroutineScopedResource() {

    @Inject
    lateinit var personService: CoroutinesPersonService

    @GET
    @Path("/persons/{personId}")
    fun getPerson(@PathParam("personId") personId: UUID): Uni<PersonDto> {
        return asyncUni {
            personService.getPerson(personId) ?: throw NotFoundException("Person with id $personId not found.")
        }
    }

    @POST
    @Path("/persons")
    fun savePerson(person: PersonDto): Uni<PersonDto> {
        return asyncUni {
            personService.createPerson(person)
        }
    }
}
